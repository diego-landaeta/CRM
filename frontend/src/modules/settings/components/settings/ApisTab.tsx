  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-body text-muted-foreground mt-0.5">Credenciales compartidas por todos los proyectos (ej: Claude AI para reportes). Las APIs por proyecto se configuran en Proyectos &gt; Configurar &gt; APIs.</p>
      </div>
...
function CredentialCard({ service, projectName, credential, onConfigure, onTest, onDelete }) {
  return (
    <div className="bg-card p-5 rounded-lg border border-border flex items-center gap-4">
      ...
      {credential ? (
        <>
          <span className={`px-2.5 py-1 rounded-full text-micro font-medium ${
        ...
        <>
          <span className="px-2.5 py-1 rounded-full text-micro font-medium bg-amber-50 text-amber-600 dark:bg-amber-950/30 dark:text-amber-400">sin configurar</span>
      ...
      <div className="fixed inset-0 !m-0 z-[80] flex items-center justify-center sm:p-4">
        ...
        <div className="relative bg-card rounded-lg border border-border w-full max-w-md p-6">
          ...
          <form onSubmit={handleSave} className="space-y-3">
            <div>
              <label className="text-meta font-medium text-muted-foreground mb-1 block">API Key / Token *</label>
              ...
              <p className="text-micro text-muted-foreground mt-1">Se cifra con AES-256-GCM antes de guardar</p>
            </div>
            <div>
              <label className="text-meta font-medium text-muted-foreground mb-1 block">Metadata (JSON opcional)</label>
