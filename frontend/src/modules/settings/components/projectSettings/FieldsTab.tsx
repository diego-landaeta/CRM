            ...
            return (
              <div key={bf.key} className="flex items-center justify-between px-4 py-2.5 text-sm">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="font-mono text-micro text-muted-foreground">{bf.key}</span>
                  {bf.alwaysRequired && <span className="px-1.5 py-0.5 rounded text-micro font-bold bg-red-50 text-red-600 dark:bg-red-950/40 dark:text-red-300">SIEMPRE REQ</span>}
                </div>
      ...
      {view === 'editor' ? (
        <>
          <form onSubmit={handleAdd} className="p-4 bg-muted/30 rounded-md border border-border">
            <p className="text-meta font-medium text-muted-foreground mb-3">Nuevo campo</p>
                      ...
                      {isEditing ? (
                        ...
                        <>
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="font-mono text-micro text-muted-foreground">{f.field_key}</span>
                            <span className="px-1.5 py-0.5 rounded text-micro font-medium bg-primary/10 text-primary">{FIELD_TYPES.find(t => t.v === f.type)?.label || f.type}</span>
                            {f.required && <span className="px-1.5 py-0.5 rounded text-micro font-bold bg-red-50 text-red-600">REQ</span>}
                            {f.grupo && <span className="px-1.5 py-0.5 rounded text-micro font-bold bg-amber-50 text-amber-700">{f.grupo}</span>}
                          </div>
                          {Array.isArray(f.options) && f.options.length > 0 && (
                            <p className="text-meta text-muted-foreground mt-1 truncate">Opciones: {f.options.join(', ')}</p>
                          )}
                        </>
                      )}
                    </div>
        ...
        <div className="space-y-4">
          ...
          {Object.keys(groups).length === 0 ? (
            ...
            <div key={grupo} className="bg-muted/20 rounded-md p-4 border border-border">
              <p className="text-meta font-medium text-muted-foreground mb-3">{grupo}</p>
