  ...
  return (
    <div className="space-y-5 max-w-3xl">
      ...
      <form onSubmit={handleAdd} className="p-4 bg-muted/30 rounded-md border border-border">
        <p className="text-meta font-medium text-muted-foreground mb-3">Nuevo panel</p>
                  ...
                  {isEditing && editBuf ? (
                    ...
                    <>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="px-1.5 py-0.5 rounded text-micro font-bold bg-primary/10 text-primary">
                          {p.open_in === 'tab' ? 'Pestaña' : 'Iframe'}
                        </span>
                        {p.icon && <span className="font-mono text-micro text-muted-foreground">{p.icon}</span>}
                      </div>
                      <a href={p.url} target="_blank" rel="noopener noreferrer" className="text-meta text-muted-foreground hover:text-primary truncate block mt-0.5">
                        {p.url}
                      </a>
                    </>
                  )}
