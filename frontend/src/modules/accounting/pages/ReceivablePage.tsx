                ...
                <ul className="space-y-1.5">
                  {itemsDelDia.map(r => (
                    <li key={`${r.tipo}-${r.ref_id}`}>
                      <button onClick={() => navigate(`/prospectos/${r.lead_id}`)} className="w-full text-left flex items-center justify-between gap-2 rounded-md border border-border px-3 py-2 hover:bg-muted/40">
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {visibles.map(r => (
                  <tr key={`${r.tipo}-${r.ref_id}`} className="border-b last:border-0 hover:bg-muted/30 cursor-pointer" onClick={() => navigate(`/prospectos/${r.lead_id}`)}>
          ...
          <div className="md:hidden divide-y divide-border">
            {visibles.map(r => (
              <button key={`${r.tipo}-${r.ref_id}`} type="button" onClick={() => navigate(`/prospectos/${r.lead_id}`)} className="w-full text-left p-4 space-y-1.5 hover:bg-muted/30">
