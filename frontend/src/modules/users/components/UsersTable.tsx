                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-semibold flex-shrink-0 ${avatarColorFor(u.id)}`}>
          ...
          return (
            <div key={u.id} className={`p-4 space-y-2 ${!u.active ? 'opacity-50' : ''}`}>
              <div className="flex items-center gap-2.5">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-semibold ${avatarColorFor(u.id)}`}>
                  {getInitials(u.nombre)}
                </div>
                <div className="flex-1 min-w-0">
                  <span className="text-[13px] font-semibold block truncate">{u.nombre || 'Sin nombre'}</span>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <EstadoBadge user={u} />
                <AusenciaBadge estado={disponibilidad.get(u.id)} />
                <span className="text-[10px] text-muted-foreground truncate max-w-[200px]" title={proyectos}>{proyectos}</span>
                {u.last_login_at ? (
