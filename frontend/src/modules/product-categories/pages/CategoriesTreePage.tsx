...
function NodeRow({
  ...
  // Filtros: si este nodo no matchea pero algún descendiente sí, aún lo mostramos
  const matchesSelf =
      ...
      <div className="bg-card border rounded-lg shadow-lg w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b">
          <h3 className="font-semibold">
            {isEdit ? 'Editar categoría' : state.parent_id ? 'Nueva subcategoría' : 'Nueva categoría raíz'}
          </h3>
...
export default function CategoriesTreePage() {
  ...
  async function reload() {
    ...
    try {
      ...
      setError((e as Error).message || 'Error al cargar el árbol');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleDelete(node: CategoryNode) {
    const msg = node.children.length > 0
      ? `"${node.nombre}" tiene ${node.children.length} subcategorías. ¿Borrar de todas formas?`
      : `¿Borrar "${node.nombre}"?`;
    ...
    <div className="container mx-auto py-6 max-w-5xl">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4 p-3 border rounded-lg bg-muted/30">
        ...
        >
          ...
          <option value="wp_menu">Solo del menú WP</option>
        </select>
      ...
      {!loading && !error && tree.length === 0 && (
        <div className="bg-muted rounded p-8 text-center text-muted-foreground">
          Sin categorías en este proyecto.
          <br />
      ...
      {!loading && tree.length > 0 && (
        <>
          <div className="text-xs text-muted-foreground mb-3">
            <strong>{stats.total}</strong> categorías · <strong>{stats.productos}</strong> productos asignados
          </div>
