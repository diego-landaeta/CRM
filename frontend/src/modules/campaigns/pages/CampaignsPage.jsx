...
const STATUS_STYLES = {
  ...
  pausada: 'bg-zinc-100 text-zinc-500',
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-zinc-500 text-sm">Metricas de rendimiento &mdash; {activeProject.nombre}</p>
      </div>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[13px] font-bold transition-all ${
            tab === 'meta' ? 'bg-[#4361ee] text-white shadow-lg shadow-[#4361ee]/20' : 'border border-zinc-200 bg-white text-zinc-600 hover:bg-zinc-50'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[13px] font-bold transition-all ${
            tab === 'google' ? 'bg-[#4361ee] text-white shadow-lg shadow-[#4361ee]/20' : 'border border-zinc-200 bg-white text-zinc-600 hover:bg-zinc-50'
          }`}
        >
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-zinc-100 shadow-[0_1px_2px_0_rgb(0_0_0/0.05)] text-center">
          <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">Total Gastado</p>
        ...
        <div className="bg-white p-5 rounded-2xl border border-zinc-100 shadow-[0_1px_2px_0_rgb(0_0_0/0.05)] text-center">
          <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">CPL Medio</p>
      ...
      {campaigns.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {campaigns.map((c) => (
            <div key={c.id} className="bg-white p-6 rounded-3xl border border-zinc-100 shadow-[0_1px_2px_0_rgb(0_0_0/0.05)] hover:shadow-[0_10px_15px_-3px_rgb(0_0_0/0.04)] hover:-translate-y-0.5 transition-all">
              <div className="flex items-start justify-between mb-5">
                <div>
                  <h3 className="font-bold tracking-tight">{c.name}</h3>
                  <p className="text-[11px] text-zinc-400 mt-1">{c.dates}</p>
                </div>
                <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${STATUS_STYLES[c.status]}`}>{c.status}</span>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-zinc-50 rounded-xl py-3 text-center"><p className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">Gastado</p><p className="font-extrabold text-base mt-0.5">{c.spent.toLocaleString('es-ES')} &euro;</p></div>
                <div className="bg-zinc-50 rounded-xl py-3 text-center"><p className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">Leads</p><p className="font-extrabold text-base mt-0.5">{c.leads}</p></div>
