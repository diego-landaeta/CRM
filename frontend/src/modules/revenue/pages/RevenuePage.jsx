    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <div className="bg-card p-6 rounded-lg border-border">
          ...
          <p className="text-muted-foreground text-xs text-muted-foreground">Ingresos Mes</p>
          <h3 className="text-3xl font-semibold mt-1">{(stats.ingresosMes || 0).toLocaleString('es-ES')} &euro;</h3>
        </div>
        <div className="bg-card p-6 rounded-lg border-border">
          <div className="flex items-center justify-between mb-4">
          ...
          <p className="text-muted-foreground text-xs text-muted-foreground">Conversiones Mes</p>
          <h3 className="text-3xl font-semibold mt-1">{stats.convertido || 0}</h3>
        </div>
        <div className="bg-card p-6 rounded-lg border-border">
          <div className="flex items-center justify-between mb-4">
          ...
          <p className="text-muted-foreground text-xs text-muted-foreground">Ticket Medio</p>
          <h3 className="text-3xl font-semibold mt-1">{ticketMedio} &euro;</h3>
        </div>
        <div className="bg-card p-6 rounded-lg border-border">
          <div className="flex items-center justify-between mb-4">
            <div className="w-10 h-10 rounded-md bg-amber-50 flex items-center justify-center text-amber-600"><Hourglass size={20} weight="regular" /></div>
          </div>
          <p className="text-muted-foreground text-xs text-muted-foreground">CPL Medio</p>
          <h3 className="text-3xl font-semibold mt-1">{stats.cplMedio || 0} &euro;</h3>
        </div>
      </div>
                ...
                <tbody>
                  {conversions.map((c) => (
                    <tr key={c.id} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                      ...
                      <td className="px-5 py-3"><span className={`px-2.5 py-1 rounded-full text-[10px] font-medium ${TIPO_STYLES[c.tipo]}`}>{c.tipo === 'pago_completo' ? 'Completo' : 'Parcial'}</span></td>
              ...
              {conversions.map((c) => (
                <div key={c.id} className="p-4 space-y-2">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-medium ${TIPO_STYLES[c.tipo]}`}>{c.tipo === 'pago_completo' ? 'Completo' : 'Parcial'}</span>
