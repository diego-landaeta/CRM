...
// Lista de reportes de spam pendientes. Solo visible al superadmin.
// Cada reporte se puede confirmar (soft-delete el lead) o descartar.
export default function SpamReportsSection() {
  ...
  const fetchReports = useCallback(async () => {
    setLoading(true);
    try {
      const res = await client.get('/leads/spam-reports');
      if (res.success) setReports(res.data || []);
    } catch (err: any) {
      // 403 si no es superadmin; lo absorbemos
      if (err?.status !== 403) toast({ title: 'Error', description: err?.message, variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  if (loading) {
    return <div className="text-sm text-muted-foreground p-4">Cargando reportes…</div>;
  }
            ...
            <div key={r.id} className="bg-card border border-border rounded-lg p-3">
              <div className="flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  {r.motivo && (
                    <p className="text-xs mt-1.5 p-2 rounded bg-muted/50 italic">"{r.motivo}"</p>
                  )}
                </div>
