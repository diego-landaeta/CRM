...
export default function MakeWebhookDetailPage() {
  ...
  async function changeMode(mode: 'test' | 'active') {
    try {
      ...
      if (res.success) {
        toast({ title: `Modo ${mode === 'active' ? 'ACTIVO — creará leads' : 'TEST — solo guarda payload'}` });
        await load();
      }
    } catch (err: any) {
      toast({ title: 'Error', description: err?.data?.error, variant: 'destructive' });
  ...
  async function rotateSecret() {
    if (!confirm('¿Rotar el secret? Make dejará de funcionar hasta que actualices el header X-Make-Secret en tu escenario.')) return;
    try {
      const res = await client.post(`/make-webhooks/${id}/rotate-secret`, {});
      if (res.success) {
        toast({ title: 'Secret rotado', description: 'Actualízalo en tu escenario de Make' });
        await load();
      }
    } catch (err: any) {
      toast({ title: 'Error', description: err?.data?.error, variant: 'destructive' });
  ...
  const baseUrl = window.location.origin + (import.meta.env.BASE_URL || '/crm/').replace(/\/$/, '');
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <div className="bg-card border border-border rounded-lg p-4">
        <p className="text-xs font-semibold text-muted-foreground mb-2">Modo de operación</p>
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">Solo guarda el payload para inspección. NO crea leads.</p>
          </button>
          <button
      ...
      <div className="bg-card border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-md p-3 text-[11px] text-blue-800 dark:text-blue-300">
          <p className="font-semibold mb-1">Configuración en Make:</p>
          <ol className="list-decimal list-inside space-y-0.5">
            <li>Módulo "HTTP → Make a request"</li>
            ...
            <li>Lanza un test → vuelve aquí para ver el payload capturado y mapear</li>
          </ol>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <div className="bg-card border border-border rounded-lg p-4">
          <p className="text-xs font-semibold text-muted-foreground mb-2">
            Último payload recibido {hook.sample_received_at && `(${new Date(hook.sample_received_at).toLocaleString('es-ES')})`}
          </p>
          {hook.sample_payload ? (
            ...
            <p className="text-xs text-muted-foreground italic">Aún no hemos recibido nada. Lanza un test desde Make.</p>
          )}
        </div>
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          ...
          <div className="border-t border-border mt-3 pt-3">
            <p className="text-[11px] font-semibold text-muted-foreground mb-1">Añadir campo</p>
            <div className="flex gap-1">
              <select
                value={newField}
      ...
      <div className="bg-card border border-border rounded-lg p-4">
        <p className="text-xs font-semibold text-muted-foreground mb-2 flex items-center gap-2">
          <PaperPlaneTilt size={14} /> Historial de envíos (últimos 20)
        </p>
        {deliveries.length === 0 ? (
          <p className="text-xs text-muted-foreground italic">Sin envíos aún.</p>
        ) : (
          <div className="space-y-1">
            {deliveries.map(d => (
              <div key={d.id} className="flex items-center gap-2 text-[11px] py-1 border-b border-border/60 last:border-0">
                ...
                {d.lead_id && <a href={`/crm/leads/${d.lead_id}`} className="text-primary underline">lead #{d.lead_id}</a>}
