// Diálogo de descarga de reportes con rango de fechas. Lo abre el desplegable
// "Reportes" de Prospectos. Dos modos:
//  - 'leads'  → Prospectos + Ventas (incluye convertidos)
//  - 'ventas' → Ventas (conversiones)
import { useState } from 'react';
import { X, FileXls, FileCsv, CalendarBlank } from '@phosphor-icons/react';
import Portal from '@/shared/components/ui/portal';
import { toast } from '@/shared/hooks/useToast';
import { downloadLeadsReport, downloadVentasReport } from '../lib/leadsReport';
...
export default function ReportDownloadDialog({ open, onClose, mode, projectId, projectName }: Props) {
  ...
  async function descargar(format: ExportFormat) {
    ...
    try {
      ...
      const n = esVentas
        ? await downloadVentasReport({ projectId, dateFrom, dateTo }, { format, filename: base })
        : await downloadLeadsReport({ projectId, dateFrom, dateTo, includeConverted: true }, { format, filename: base });
      toast({ title: 'Descarga lista', description: `${n} registro${n === 1 ? '' : 's'} exportado${n === 1 ? '' : 's'}.` });
      onClose();
    } catch (err) {
      toast({ title: 'No se pudo generar el reporte', description: (err as { message?: string })?.message, variant: 'destructive' });
    } finally {
      setBusy(null);
    }
  }
          ...
          <div className="p-4 space-y-4">
            <p className="text-xs text-muted-foreground">
              {esVentas
                ...
                : 'Todos los prospectos del período, incluyendo los que ya son venta (aparecen con estado “Convertido”).'}
            ...
            <div className="flex items-center gap-2 pt-1">
              <button type="button" disabled={busy !== null} onClick={() => descargar('xlsx')}
              ...
              <button type="button" disabled={busy !== null} onClick={() => descargar('csv')}
              ...
              <span className="ml-auto text-[11px] text-muted-foreground">{projectName || 'Todos los proyectos'}</span>
            </div>
          </div>
        </div>
      </div>
