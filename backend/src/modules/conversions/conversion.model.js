...
export async function create(data) {
  ...
  try {
    ...
    if (Number(importe_pagado) > 0) {
      // Método REAL del pago inicial (no 'fraccionado', que es el plan). Se toma
      // el elegido (metodo_pago_inicial); si no, el método simple; y para
      // fraccionado se asume Tarjeta.
      const metodoInicial = metodo_pago_inicial
        || (metodo_pago && metodo_pago !== 'fraccionado' ? metodo_pago : 'tarjeta');
      const { rows: firstPay } = await client.query(
        ...
        [conversion.id, importe_pagado, fecha_conversion, 'Pago inicial', metodoInicial]
      );
...
export async function findAll({ projectId, projectIds = null, leadId, responsableId, pendiente, vencido, pendingBilling, producto, from, to, page, limit }) {
  ...
  */
  ...
  // de las ventas, y hasta ahora no estaba en ninguna parte de esta pantalla:
  // la tarjeta se llamaba «Facturado» y enseñaba la suma de las ventas, que el
  // 8/9 en ISEIH daba 200 EUR cuando ese dia se emitieron 317,50 EUR en
  // facturas. De ahi que las dos pantallas parecieran contradecirse.
 ...
 * factura es dinero cobrado que no se ha declarado, y hasta ahora esa lista no
 * existía en ninguna pantalla.
 *
 * Misma definición de cuota que en los totales —cualquier cobro que no sea el
 * primero de su venta—, para que el desglose sume exactamente lo que dice la
 * tarjeta.
 */
export async function cuotasDelPeriodo({
  ...
  const lista = comoLista(projectId, projectIds);
  let alcance = SIN_PRUEBAS('c.project_id');
  if (lista) { args.push(lista); alcance = `c.project_id = ANY($${args.length}::int[])`; }
  let porGestora = '';
  if (responsableId) {
