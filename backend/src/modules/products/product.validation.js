...
// #86 · Convocatoria. Solo se guardan los dos extremos: cuantas plazas hay y
// cuantas venian ocupadas de antes. Las ocupadas de verdad y las libres las
// cuenta el modelo desde las ventas, y por eso no se aceptan por aqui: si se
// pudieran teclear, alguien las tecleara y el numero dejara de ser cierto.
//
// El '' que llega de un campo vacio del formulario se convierte en null, que es
// lo que significa: «esta formacion no lleva cuenta de plazas».
const vacioEsNulo = (v) => (v === '' || Number.isNaN(v) ? null : v);
// La columna es NOT NULL DEFAULT 0, asi que un vacio no puede acabar en null:
// «ninguna» es cero.
//
// `undefined` se deja pasar tal cual, y esa distincion importa: undefined es
// «el cliente no manda este campo» y tiene que seguir siendo undefined para que
// `update()` no lo toque. Si aqui se convirtiera a 0, cada vez que alguien
// guardara un producto por cualquier otro motivo se le borraria el contador.
