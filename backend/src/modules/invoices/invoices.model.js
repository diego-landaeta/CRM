...
export async function create(data, userId) {
  ...
  try {
    ...
    // Serie de la empresa emisora (o la del request, o 'A'). Decisión de negocio:
    // las PROFORMAS comparten el MISMO correlativo que las facturas normales
    // (misma serie y mismo contador), así que una proforma toma el siguiente
...
export async function deleteInvoice(id) {
  ...
  try {
    ...
    // Reajusta el correlativo de la serie (issuer_id, ano, serie) al máximo restante.
...
export async function emitirFacturaDePago(conversionId, { paymentId, importe }, userId = null) {
  ...
  // arranque — p.ej. CEDIA empezó a facturar el 18/7: un cobro de junio no lleva
  // factura. Si la sociedad aún no factura, no bloquea (permite el primer arranque).
  if (fechaPago) {
  ...
  // El programa viene del catálogo (no del texto libre, que puede traer el nombre del cliente).
