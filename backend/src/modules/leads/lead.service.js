...
async function _createLeadCore(project, leadData) {
  ...
  // mantiene limpia la cola del gestor. Si llega del mismo email pero OTRO
  // producto, sigue el flujo normal de reincidente (lead nuevo enlazado).
...
export async function changeStatus(leadId, newStatus, motivo, userId) {
  ...
  // Si va a no_interesado con motivo, guardar también como nota en interacciones
  // para que el equipo vea el motivo en el feed (no solo en status_history).
