...
export const BUNDLES = {
  core: {
    ...
    modules: ['auth', 'users', 'projects', 'credentials', 'field-definitions'],
    requires: [],
  },
  leads: {
    label: 'Leads / Captacion',
    description: 'Pipeline + audiencias + forms + webhooks',
    modules: ['leads', 'forms', 'webhook-tokens', 'audiences'],
    requires: ['core'],
  },
  comercial: {
    label: 'Comercial / Ventas',
    description: 'Productos + conversiones + comisiones + matriculas + dossiers',
    modules: ['products', 'product-categories', 'conversions', 'commissions', 'matriculas', 'dossiers'],
    requires: ['core'],
  },
  accounting: {
    label: 'Contabilidad',
